...
TODO: change github description
...
Execute the following from shell, assuming you have already installed and configured the GCP TODO SDK.
...
Execute the following from shell, assuming you have already installed and configured the AWS TODO SDK (for a subset of people you can use MDB's SSO to access AWS account to obtain the SSO start URL):
...
The following table shows the results of using this project with various LLMs against the codebase of the legacy Sun/Oracle Java Petstore J2EE application TODO: URL (tested on 17-July-2025):
...
* **Reactively Crop Excessively Large Prompt**.  If the LLM response error indicates the prompt is too large, it shortens the prompt to the size instructed by the LLM's API response, if specified. It otherwise, estimates the appropriate shorter size and applie that. Note, it only crops a prompt of its been unable to TODO fulfill the full prompt with the backup secondary LLM first, in case that has a larger accomodating limit.
