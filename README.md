...
1. Ensure you have the codebase for a sample application ready to access on your local filesystem. You can also [download a zip of some example projects](https://drive.google.com/file/d/1rDSOiLOH0xq3Hc5k8i3DvZpCDVvDccr1/view?usp=sharing) for testing (for those that have access). Note the current version of these tools work better with Java-based codebases, but over time, many more programming languages will be supported equally.
