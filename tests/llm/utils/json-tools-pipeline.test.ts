...
describe('json-tools sanitation pipeline (incremental refactor wrapper)', () => {
  ...
  const result = parseAndValidateLLMJsonContent(malformed, 'resilient-path', completionOptions, true);
  ...
  const result = parseAndValidateLLMJsonContent(withConcat, 'pre-concat', completionOptions, true);
