import { extractBalancedJsonThenParse } from "../../../src/llm/json-processing/json-extractor";

describe("json-extractor", () => {
  describe("extractBalancedJsonThenParse", () => {
    it("should parse valid JSON object", () => {
      const input = '{"name": "John", "age": 30}';
      const result = extractBalancedJsonThenParse(input);
    ...
    it("should throw error when no JSON content is found", () => {
      ...
      expect(() => extractBalancedJsonThenParse(input)).toThrow("No JSON content found");
    });

    it("should throw JsonParseFailed for malformed JSON", () => {
      const input = '{"invalid": json content}';
      expect(() => extractBalancedJsonThenParse(input)).toThrow("JsonParseFailed");
    });
