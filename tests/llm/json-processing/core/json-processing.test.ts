...
describe("json-processing", () => {
  ...
  describe("processJson", () => {
    ...
    describe("type preservation", () => {
      it("should preserve type when using generic processJson with schema", () => {
        const schema = z.object({ name: z.string(), age: z.number() });
        const json = '{"name": "John", "age": 30}';
        const completionOptions = {
          outputFormat: LLMOutputFormat.JSON,
        ...
        expect(result.success).toBe(true);
        if (result.success) {
          // Type check: result.data should be strongly typed
          const typedData: z.infer<typeof schema> = result.data;
          expect(typedData.name).toBe("John");
          expect(typedData.age).toBe(30);
          // Verify no 'any' type leakage
          expect(typeof typedData.name).toBe("string");
          expect(typeof typedData.age).toBe("number");
        }
      });

      it("should preserve type through validateJsonWithTransforms", () => {
        const schema = z.object({
          id: z.number(),
          tags: z.array(z.string()),
        });
        const json = '{"id": 42, "tags": ["tag1", "tag2"]}';
        const completionOptions = {
          outputFormat: LLMOutputFormat.JSON,
          jsonSchema: schema,
        };
        ...
        expect(result.success).toBe(true);
        if (result.success) {
          // Type should be preserved from schema inference
          const typedData: z.infer<typeof schema> = result.data;
          expect(typedData.id).toBe(42);
          expect(Array.isArray(typedData.tags)).toBe(true);
          expect(typedData.tags).toEqual(["tag1", "tag2"]);
        }
      });

      it("should preserve type even when transforms are applied", () => {
        const schema = z.object({
          name: z.string(),
          items: z.array(z.string()).optional(),
        });
        // JSON with null that will be transformed to undefined
        const json = '{"name": "test", "items": null}';
        const completionOptions = {
          outputFormat: LLMOutputFormat.JSON,
          jsonSchema: schema,
        };
        ...
        expect(result.success).toBe(true);
        if (result.success) {
          // Type should still be preserved after transforms
          const typedData: z.infer<typeof schema> = result.data;
          expect(typedData.name).toBe("test");
          // items should be undefined (null converted to undefined)
          expect(typedData.items).toBeUndefined();
        }
      });
      ...
      it("should default to Record<string, unknown> when no schema provided", () => {
        ...
        expect(result.success).toBe(true);
        if (result.success) {
          // Type should default to Record<string, unknown>
          const typedData: Record<string, unknown> = result.data;
          expect(typedData.key).toBe("value");
          expect(typedData.number).toBe(42);
        }
      });
    });
  });
