...
import type { LLMContext, LLMGeneratedContent } from "../../../src/llm/types/llm.types";
...
describe("LLMErrorLogger", () => {
  ...
  describe("recordJsonProcessingError", () => {
    ...
    it("should write error log file with correct content", async () => {
      ...
      expect(mockEnsureDirectoryExists).toHaveBeenCalledWith("output/errors");
      expect(mockWriteFile).toHaveBeenCalled();
      const writeCall = mockWriteFile.mock.calls[0];
      expect(writeCall[0]).toMatch(/output\/errors\/response-error-.*\.log/);
