...
/**
 * Checks if text looks like conversational filler that LLMs add to their responses.
 * This uses a generic pattern-based approach to detect sentence-like structures
 * that start with capitalized words followed by more words.
 *
 * Detection patterns:
 * 1. Starts with a capitalized word followed by optional comma and another word
 * 2. Contains multiple words (indicates sentence structure)
 * 3. Is long enough to be meaningful text (not a short enum value)
 *
 * Safeguards:
 * - Minimum length check (>10 chars) to avoid matching short enum values
 * - Must contain spaces (indicating multi-word content)
 * - Must not contain JSON structural characters
 *
 * @param text - The text to check
 * @returns True if the text looks like conversational filler
 *
 * @example
 * looksLikeConversationalFiller("Note that this is important") // true
 * looksLikeConversationalFiller("Also, we should consider") // true
 * looksLikeConversationalFiller("However the implementation") // true
 * looksLikeConversationalFiller("OK") // false (too short)
 * looksLikeConversationalFiller("MyEnumValue") // false (no spaces)
 */
export function looksLikeConversationalFiller(text: string): boolean {
  ...
  // Must contain spaces (indicates multi-word content, not an identifier)
  if (!trimmed.includes(" ")) {
    return false;
  }

  // Minimum length check to avoid matching short enum values or abbreviations
  if (trimmed.length <= 10) {
    return false;
  }

  // JSON keywords should never be considered filler
  if (isJsonKeyword(trimmed)) {
    return false;
  }

  // Count words for later checks
  const wordCount = trimmed.split(/\s+/).filter((w) => w.length > 0).length;

  // Pattern: Starts with capitalized word (first letter uppercase, rest lowercase),
  // optional comma, then lowercase word(s)
  // Examples: "Note that...", "Also, we...", "However the...", "Additionally, this..."
  // This is a generic sentence start pattern
  // Must have at least 3 words to be considered filler (to avoid "Simple type" etc.)
  // Note: No "i" flag - we specifically want to match only capitalized first words
  if (/^[A-Z][a-z]+(?:,)?\s+[a-z]+/.test(trimmed) && wordCount >= 3) {
    return true;
  }

  // Pattern: Common transitional/filler phrase starters with 5+ letter first word
  // These are structural patterns (adverb/conjunction + content) rather than hardcoded words
  // Matches: "Furthermore,...", "Moreover,...", "Therefore,...", etc.
  // Note: No "i" flag - we specifically want to match only capitalized first words
  if (/^[A-Z][a-z]{4,}(?:,|\s)/.test(trimmed) && wordCount >= 3) {
    return true;
  }
