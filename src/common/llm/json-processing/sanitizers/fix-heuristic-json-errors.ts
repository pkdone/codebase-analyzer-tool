...
import { LLMSanitizerConfig } from "../../config/llm-module-config.types";
 ...
 */
export const fixHeuristicJsonErrors: Sanitizer = (
  input: string,
  ...
  try {
    ...
    // Extract config values with defaults - only used for COMMON_PROPERTY_STARTS
    const propertyNameMappings = config?.propertyNameMappings ?? {};
    ...
    sanitized = sanitized.replace(
      ...
      (match, whitespace, truncated, offset: number) => {
        ...
        const fixedName = propertyNameMappings[truncatedStr];
